...
export function DashboardMetrics({ runs, workflowCount }: DashboardMetricsProps) {
  ...
  const getStatusBadge = (status: RunStatus) => {
    switch (status) {
      ...
      case "EXECUTING":
        return (
          <Badge variant="outline" className="bg-violet-500/10 text-violet-500 border-violet-500/20 gap-1 animate-pulse font-medium">
  ...
  return (
    <div className="flex-1 space-y-8 p-8 pt-6 overflow-y-auto max-h-screen">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="relative overflow-hidden transition-all hover:shadow-md border bg-card/60 backdrop-blur-sm group">
          <div className="absolute top-0 left-0 h-full w-[4px] bg-violet-500" />
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">
              Total Executions
            </CardTitle>
            <Play className="size-4 text-violet-500 transition-transform group-hover:scale-110" />
          </CardHeader>
          <CardContent>
